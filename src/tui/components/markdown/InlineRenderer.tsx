...
const RenderInlineInternal: React.FC<RenderInlineProps> = ({
    text, textColor, colors: colorsProp,
}) => {
    const theme = useTheme();
    const colors = colorsProp ?? theme.colors;
    const fg = textColor ?? colors.text.primary;
    // 快速检测：如果不含任何 Markdown 标记，直接返回纯文本
    if (!/[*_~`\[]|https?:|<u>/.test(text)) {
        ...
        try {
            if (
                ...
                if (codeMatch && codeMatch[2]) {
                    renderedNode = (
                        <Text key={key} color={colors.text.code}>
                            {codeMatch[2]}
                        </Text>
                    );
                }
                ...
                if (linkMatch) {
                    ...
                    renderedNode = (
                        <Text key={key}>
                            {linkText}
                            <Text color={colors.text.link}> ({url})</Text>
                        </Text>
                    );
                }
            } else if (
                ...
                renderedNode = (
                    <Text key={key} color={colors.text.link}>
                        {fullMatch}
                    </Text>
                );
            }
    ...
    if (lastIndex < text.length) {
        nodes.push(<Text key={`t-${lastIndex}`} color={fg}>{text.slice(lastIndex)}</Text>);
    }
